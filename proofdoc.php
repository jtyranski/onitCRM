...
$leak_id = $_GET['leak_id']; // pass the leak_id to it.
...
date_format(a.eta_date, \"%m/%d/%Y %h:%i %p\") as eta_pretty
from am_leakcheck a, am_users e
where a.user_id = e.user_id and leak_id='$leak_id'";
$result = executequery($sql);
...
$x += 60;
$pdf->SetX($x);
$pdf->Cell(0, 0, "Service Dispatch " . $leak_id, 0, 1, 'L');
$pdf->SetX($x);
$pdf->Cell(0, 0, "Distributed By: " . stripslashes($leak['dispatchedby']), 0, 1, 'L');
$pdf->SetX($x);
$pdf->Cell(0, 0, "Date: " . stripslashes($leak['dispatch_pretty']), 0, 1, 'L');
$pdf->SetX($x);
$pdf->Cell(0, 0, "ETA: " . stripslashes($leak['eta_pretty']), 0, 1, 'L');
...
$pdf->Cell(0, 0, "Hours of Operation:", 0, 1, 'L');
if($company['hours_of_operation'] != ""){
  $pdf->MultiCell(0, 0, $company['hours_of_operation'] . "\n\n", 0, 'L', false, 1);
}
else {
  $pdf->MultiCell(0, 0, "\n\n\n", 0, 'L', false, 1);
}

$pdf->Cell(0, 0, "Check In Procedure:", 0, 1, 'L');
if($company['checkin_procedure'] != ""){
  $pdf->MultiCell(0, 0, $company['checkin_procedure'] . "\n\n", 0, 'L', false, 1);
}
else {
  $pdf->MultiCell(0, 0, "\n\n\n", 0, 'L', false, 1);
}

$pdf->Cell(0, 0, "Check Out Procedure:", 0, 1, 'L');
if($company['checkout_procedure'] != ""){
  $pdf->MultiCell(0, 0, $company['checkout_procedure'] . "\n\n", 0, 'L', false, 1);
}
else {
  $pdf->MultiCell(0, 0, "\n\n\n", 0, 'L', false, 1);
}

$pdf->Cell(0, 0, "Invoice Requirements:", 0, 1, 'L');
if($company['invoice_requirements'] != ""){
  $pdf->MultiCell(0, 0, $company['invoice_requirements'] . "\n\n", 0, 'L', false, 1);
}
else {
  $pdf->MultiCell(0, 0, "\n\n", 0, 'L', false, 1);
}

...
function category_tree($category_id) {
	...
	if (!$query) echo "Database Error : ".$q;
		...
		while ($row=mysql_fetch_row($query)) {
			...
			if (!$supercategory) {
				$q = "select moderate from ".$table_prefix."users_to_categories where category_id = ".$row[0]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
				$qu = mysql_query($q);
...
function group_tree($group_id) {
	...
	if (!$query) echo "Database Error : ".$q;
		...
		while ($row=mysql_fetch_row($query)) {
			...
			if (!$supergroup) {
				$q = "select moderate from ".$table_prefix."users_to_groups where group_id = ".$row[0]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
				$qu = mysql_query($q);
				if (mysql_num_rows($qu) > 0) $mod = mysql_result($qu,0,0);
			} else {
				$mod = 3;
...
if (!$superview) {
	...
	if ($_POST["mode"] == $lang["add_event"]) {
		if(get_magic_quotes_gpc()) {
            $title = go_escape_string(stripslashes($_POST["title"]));
			$link_id = go_escape_string(stripslashes($_POST["venue"]));
			$contact_id = go_escape_string(stripslashes($_POST["contact"]));
			$description = go_escape_string(stripslashes($_POST["description"]));
			$category_id = go_escape_string(stripslashes($_POST["category"]));
			$group_id = go_escape_string(stripslashes($_POST["group"]));
			$propose = go_escape_string(stripslashes($_POST["propose"]));
			$prospect_id = go_escape_string(stripslashes($_POST["prospect_id"]));
			$property_id = go_escape_string(stripslashes($_POST["property_id"]));
			$what = go_escape_string(stripslashes($_POST["what"]));
			$value = go_escape_string(stripslashes($_POST["value"]));
			$contactlist = $_POST["contactlist"];
        } else {
          	$title = go_escape_string($_POST["title"]);
			$link_id = go_escape_string($_POST["venue"]);
			$contact_id = go_escape_string($_POST["contact"]);
			$description = go_escape_string($_POST["description"]);
			$category_id = go_escape_string($_POST["category"]);
			$group_id = go_escape_string($_POST["group"]);
			$propose = go_escape_string($_POST["propose"]);
			$prospect_id = go_escape_string($_POST["prospect_id"]);
			$property_id = go_escape_string($_POST["property_id"]);
			$what = go_escape_string($_POST["what"]);
			$value = go_escape_string($_POST["value"]);
			$contactlist = $_POST["contactlist"];
        }
		//echo "starting test<br>";
	    if(is_array($contactlist)){
		  //echo "is an array<br>";
		  for($x=0;$x<sizeof($contactlist);$x++){
		    $contactnames .= $contactlist[$x] . ", ";
		  }
		}
		//echo $contactnames;
		//exit;
		$description .= "\n" . $contactnames;
		$sql = "SELECT ro_user_id from supercali_categories where category_id='$category_id'";
		$ro_user_id = getsingleresult($sql);
	
		
		if (!$supergroup) {
			if ($group_id) {
				$q = "select moderate from ".$table_prefix."users_to_groups where group_id = ".$group_id." and user_id = ".$SESSION_SUPERCALI_USER_ID;
				//echo $q."<br>";
		...
		if (!$supercategory) {
			$q = "select moderate from ".$table_prefix."users_to_categories where category_id = ".$category_id." and user_id = ".$SESSION_SUPERCALI_USER_ID;
		...
		$sauce = md5(time());
		$q = "INSERT into ".$table_prefix."events (title, venue_id, contact_id, description, category_id, user_id, group_id, status_id, quick_approve, prospect_id, property_id, what, value, ro_user_id) values 
		('".$title."', ".$link_id.", ".$contact_id.", '".$description."', ".$category_id.", ".$SESSION_SUPERCALI_USER_ID.", ".$group_id.", ".$status_id.", '".$sauce."', '".$prospect_id."', '".$property_id."', '".$what."','".$value."', '".$ro_user_id."')";
		$query = mysql_query($q);
...
function ajax_doACTIVITY (url, qStr) {
    url=url+"?"+qStr; 
...
function show_value_div(x){
  y = x.value;
  if(y=="BP"){
    document.getElementById('value_div').style.display="table-cell";
	document.getElementById('value_div2').style.display="table-cell";
  }
  else {
    document.getElementById('value_div').style.display="none";
	document.getElementById('value_div2').style.display="none";
  }
}
</script>
<?php
...
if($prospect_id == ""){
  $sql = "SELECT prospect_id from prospects order by company_name limit 1";
...
<table>
	...
	<td>Company</td>
	<td>
	<select name="prospect_id" onchange="ajax_doACTIVITY('add_event_property.php', 'prospect_id='+this.value)" style="width:300px;">
	<?php
	$sql = "SELECT prospect_id, company_name from prospects order by company_name";
	...
	while($record = go_fetch_array($result)){
	  ?>
	  <option value="<?=$record['property_id']?>"><?=stripslashes($record['site_name'])?></option>
	  <?php
	}
	?>
	</select>
	...
	<td>What</td>
	<td>
	<select name="what" onchange="show_value_div(this)">
	...
	<td id="value_div2">$<input type="text" name="value" size="6"></td>
	</tr>
	<?php
	...
	?>
	<tr>
	<td colspan="2" id="contacts_div">
	<?php
	if(is_array($contacts)){
	  for($x=0;$x<sizeof($contacts);$x++){
	    ?>
		<input type="checkbox" name="contactlist[]" value="<?=$contacts[$x]['name']?>"><?=$contacts[$x]['name']?><br>
		<?php
	  }
	}
	?>
	...
	<tr>
		...
		<td>
			...
			<select name="propose" id="propose"><option value="post"<?php if ($_POST["propose"] == "post") echo " selected"; ?>><?php echo $lang["post"];?></option><option value="propose"<?php if ($_POST["propose"] == "propose") echo " selected"; ?>><?php echo $lang["propose"];?></option></select>
	...
	*/ ?>
	<input type="hidden" name="venue" value="1">
...
<?php if ($fck_editor_path) {
	...
	echo "<textarea cols=\"60\" rows=\"4\" name=\"description\" id=\"description\">".$_POST["description"]."</textarea>\n";
}
?>
</p>

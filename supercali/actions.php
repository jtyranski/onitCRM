...
function check_login() {
	...
	if(get_magic_quotes_gpc()) {
		...
		$password = go_escape_string(stripslashes($_POST["password"]));
	...
	if($total_row>0){
		...
		$SESSION_SUPERCALI_USER_ID = $row['user_id'];
...
function approve($code) {
	...
	if(get_magic_quotes_gpc()) {
    	$code = go_escape_string(stripslashes($code));
     } else {
       	$code = go_escape_string($code);

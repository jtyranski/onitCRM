...
function category_table($category_id) {
	...
	if (!$query) echo "Database Error : ".$q;
		...
		while ($row=mysql_fetch_row($query)) {
			...
			if (!$edit) {
				$q = "select * from ".$table_prefix."users_to_categories where category_id = ".$row[0]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
				$qu = mysql_query($q);
...
function group_table($group_id) {
	...
	if (!$query) echo "Database Error : ".$q;
		...
		while ($row=mysql_fetch_row($query)) {
			echo "<tr><td style=\"text-align:center\">".$row[0]."</td>";
			if (!$edit) {
				$q = "select * from ".$table_prefix."users_to_groups where group_id = ".$row[0]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
				$qu = mysql_query($q);
...
while (($data = fgetcsv($fp, 10000, $_POST["separator"])) !== FALSE) {
	$start = eregi_replace("([0-9]{1,2}:[0-9]{2}):[0-9]{2}([ ]?[a|p]m)","\\1\\2",$data[6]);
	$end = eregi_replace("([0-9]{1,2}:[0-9]{2}):[0-9]{2}([ ]?[a|p]m)","\\1\\2",$data[7]);
	if (!ereg ("([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})",$data[5])) $dc = " class=\"error\"";
	if (!eregi ("([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)",$start)) $sc = " class=\"error\"";
	if (($end)&&(!eregi ("([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)",$end))) $ec = " class=\"error\"";
...
while (list($k,$v) = each($start)) {
	...
	$sc="";
	$ec="";
	if (!ereg ("([0-9]{1,2})[\/-]+([0-9]{1,2})[\/-]+([0-9]{4})",$date[$k])) $dc = " class=\"error\"";
	if (!eregi ("([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)",$dstart)) $sc = " class=\"error\"";
	if (($dend)&&(!eregi ("([0-9]{1,2}):([0-9]{2})[ ]?([a|p]m)",$dend))) $ec = " class=\"error\"";
	if ($dc|$sc|$ec) $verify .= "There are date/time errors in your listings. ";
	if (!$supercategory) {
		$q = "select * from ".$table_prefix."users_to_categories where category_id = ".$category[$k]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
		$qu = mysql_query($q);
		if (mysql_num_rows($qu) < 2) $verify = $lang["no_permision_post_events"];
	}
	if (!$supergroup) {
		$q = "select * from ".$table_prefix."users_to_groups where group_id = ".$group[$k]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
		$qu = mysql_query($q);
		if (mysql_num_rows($qu) < 2) $verify = $lang["no_permision_post_events"];
	}
	// verify categories
...
<?php
	...
	while (list($k,$v) = each($start)) {
		...
		$q = "INSERT into ".$table_prefix."events (title, venue_id, contact_id, description, category_id, user_id, group_id) values ('".$title[$k]."', ".$venue[$k].", ".$contact[$k].", '".$description[$k]."', ".$category[$k].", ".$SESSION_SUPERCALI_USER_ID.", ".$group[$k].")";
		$query = mysql_query($q);

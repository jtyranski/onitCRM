...
function showGantt ($calmonth,$calyear) {
	...
	a.what, a.act_id, a.title, a.description, a.programming_type, a.urgency
	...
	while($record = go_fetch_array($result)){
	  ...
	  $row[$counter]['programming_type'] = $programming_type;
	  $row[$counter]['urgency'] = $urgency;
	...
	foreach ($row as $key => $row2) {
	  ...
	  $data_programming_type[$key] = $row2['programming_type'];
	  $data_urgency[$key] = $row2['urgency'];
	}
	...
	for($foo=0;$foo<sizeof($row);$foo++){
	  ...
	  $programming_type = $row[$foo]['programming_type'];
	  $urgency = $row[$foo]['urgency'];
	  ...
	  if($name != $oldname){
		...
		if($ps){
		  echo "&nbsp; <a href=\"javascript:MoveAll('$user_id', 0, 'Programming')\" alt=\"Move All Back\"><img src=\"images/left.gif\" border='0'></a>";
		  echo "&nbsp; <a href=\"javascript:MoveAll('$user_id', 1, 'Programming')\" alt=\"Move All Forward\"><img src=\"images/right.gif\" border='0'></a>";
		}
		echo "</div>";
	  ...
	  if($ps) echo "<a href=\"javascript:DelEvent('" . $event_id . "')\" style=\"color:red;text-decoration:none;\">x</a> &nbsp;\n";
	  echo "<a href=" . $url . " class=\"ganttlink\"";
	  if($programming_type=="I") { echo " style=\"color:blue;text-decoration:none;\""; } else { echo " style=\"color:#CC9900;text-decoration:none;\""; } // make fixes show yellow text
	  echo ">($urgency) $title</a>\n";
	  echo "</div>";
	  if($ps){
...
<br>
<a href="javascript:load_programming('new')">+ New Project</a>
</div>
<?php
...
function programming_stage_change(x){
  if(x.value=="F"){
    document.getElementById('stage').value="Final Approved";
  }
  if(x.value=="I"){
    document.getElementById('stage').value="Requested";
  }
}
...
<div id="contact" style="position:absolute; left:100px; top:50px; z-index:151; display:none; border:2px solid #000000; padding:15px 15px 15px 15px; background-color:#FFFFFF; width:600px; height:450px; overflow:auto;" class="main">
  <div align="right">
  <a href="javascript:Closewin('contact')">X</a>
  </div>
  <form action="programming_action.php" method="post" name="programmingform" enctype="multipart/form-data">
  <div id="pform">
  </div>

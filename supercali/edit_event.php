...
if (!$superview) {
	...
	if (!$supergroup) {
		if ($row["user_id"] != $SESSION_SUPERCALI_USER_ID) {
			$q = "select moderate from ".$table_prefix."users_to_groups where group_id = ".$row[0]." and user_id = ".$SESSION_SUPERCALI_USER_ID;
...
<?php
$sql = "SELECT * from supercali_events where event_id = '" . $_REQUEST["id"] . "'";
...
<table>
	...
	<option value="BP"<?php if($what=="BP") echo " selected";?>><?=$BP?></option>
	<option value="I"<?php if($what=="I") echo " selected";?>><?=$I?></option>
	<option value="RRP"<?php if($what=="RRP") echo " selected";?>><?=$RRP?></option>
	<option value="UM"<?php if($what=="UM") echo " selected";?>><?=$UM?></option>
	<option value="O"<?php if($what=="O") echo " selected";?>>Other</option>
	</select>
	</td>
	</tr>
	<tr>
	...
	<td id="value_div2">$<input type="text" name="value" size="6" value="<?=$value?>"></td>
	</tr>
...
<script>show_value_div(document.forms['event'].what)</script>
<?php
}

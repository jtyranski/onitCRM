...
if(get_magic_quotes_gpc()) {
  	$id = go_escape_string(stripslashes($_REQUEST["id"]));
	$dir = go_escape_string(stripslashes($_REQUEST["dir"]));
	$file = go_escape_string(stripslashes($_REQUEST["file"]));

} else {
  	$id = go_escape_string($_REQUEST["id"]);
	$dir = go_escape_string(stripslashes($_REQUEST["dir"]));
	$file = go_escape_string(stripslashes($_REQUEST["file"]));
}
...
function updateProfile($id) {
	...
	if(get_magic_quotes_gpc()) {
    	$email = go_escape_string(stripslashes($_POST["email"]));
		$new_password = go_escape_string(stripslashes($_POST["new_password"]));
		
     } else {
       	$email = go_escape_string($_POST["email"]);
		$new_password = go_escape_string($_POST["new_password"]);
     }
...
function addCategory() {
	...
	if (!$edit_categories) {
		...
		if(get_magic_quotes_gpc()) {
            $name = go_escape_string(stripslashes($_POST["name"]));
			$sub_of = go_escape_string(stripslashes($_POST["sub_of"]));
			$sequence = go_escape_string(stripslashes($_POST["sequence"]));
			$description = go_escape_string(stripslashes($_POST["description"]));
			$color = go_escape_string(stripslashes($_POST["color"]));
			$background = go_escape_string(stripslashes($_POST["background"]));
        } else {
          	$name = go_escape_string($_POST["name"]);
			$sub_of = go_escape_string($_POST["sub_of"]);
			$sequence = go_escape_string($_POST["sequence"]);
			$description = go_escape_string($_POST["description"]);
			$color = go_escape_string($_POST["color"]);
			$background = go_escape_string($_POST["background"]);
        }
...
function addLink() {
	...
	if (!$edit_categories) {
		...
		if(get_magic_quotes_gpc()) {
            $company = go_escape_string(stripslashes($_POST["company"]));
			$address1 = go_escape_string(stripslashes($_POST["address1"]));
			$address2 = go_escape_string(stripslashes($_POST["address2"]));
			$city = go_escape_string(stripslashes($_POST["city"]));
			$state = go_escape_string(stripslashes($_POST["state"]));
			$zip = go_escape_string(stripslashes($_POST["zip"]));
			$phone = go_escape_string(stripslashes($_POST["phone"]));
			$fax = go_escape_string(stripslashes($_POST["fax"]));
			$contact = go_escape_string(stripslashes($_POST["contact"]));
			$email = go_escape_string(stripslashes($_POST["email"]));
			$url = go_escape_string(stripslashes($_POST["url"]));
			$description = go_escape_string(stripslashes($_POST["description"]));
        } else {
          	$company = go_escape_string($_POST["company"]);
			$address1 = go_escape_string($_POST["address1"]);
			$address2 = go_escape_string($_POST["address2"]);
			...
			$email = go_escape_string($_POST["email"]);
			$url = go_escape_string($_POST["url"]);
			$description = go_escape_string($_POST["description"]);
        }
...
function deleteEvent($id) {
	...
	if ($edit_categories) {
		$edit = true;
	} elseif ($row["user_id"] == $SESSION_SUPERCALI_USER_ID) {
		$edit = true;
	} else {
		$q = "select moderate from ".$table_prefix."users_to_categories where category_id = ".$c." and user_id = ".$SESSION_SUPERCALI_USER_ID;
	...
	if ($edit == true) {
	    $query = mysql_query("DELETE from activities_repeat where event_id=" . $id);
...
function updateModules() {
	...
	if (!$edit) {
		...
		while (list($key, $val) = each($_POST["link_name"])) {
			if(get_magic_quotes_gpc()) {
            	$key = go_escape_string(stripslashes($key));
				$val = go_escape_string(stripslashes($val));
			
        	} else {
          		$key = go_escape_string($key);
				$val = go_escape_string($val);
			...
			if ($_POST["delete"][$key]) {
				...
				if(get_magic_quotes_gpc()) {
	            	$active = go_escape_string(stripslashes($_POST["active"][$key]));
					$sequence = go_escape_string(stripslashes($_POST["sequence"][$key]));
					$year = go_escape_string(stripslashes($_POST["year"][$key]));
					$month = go_escape_string(stripslashes($_POST["month"][$key]));
					$week = go_escape_string(stripslashes($_POST["week"][$key]));
					$day = go_escape_string(stripslashes($_POST["day"][$key]));
				
	        	} else {
	          		$active = go_escape_string($_POST["active"][$key]);
					$sequence = go_escape_string($_POST["sequence"][$key]);
					$year = go_escape_string($_POST["year"][$key]);
					$month = go_escape_string($_POST["month"][$key]);
					$week = go_escape_string($_POST["week"][$key]);
					$day = go_escape_string($_POST["day"][$key]);
...
function approve($event_id) {
	...
	if (!$edit_groups) {
		$q = "select moderate from ".$table_prefix."groups_to_events where event_id = ".$event_id." and user_id = ".$SESSION_SUPERCALI_USER_ID."";
		$query = mysql_query($q);
		if (mysql_num_rows($query) > 0) {
...
if (!$SESSION_SUPERCALI_USER_ID) {
	mysql_close($link);
	header("Location: login.php");
} else {
	$query = mysql_query("SELECT add_users, add_categories, add_groups, post from ".$table_prefix."users where user_id = ".$SESSION_SUPERCALI_USER_ID." limit 1");
	...
	if (($id != $SESSION_SUPERCALI_USER_ID)&& !$post && !$edit && !$edit_categories && !$edit_groups){

...
if($submit1 != ""){
  ...
  if($test==0){
    $sql = "INSERT into unsubscribe_opm(email, opm_id) values(\"$email\", '$opm_id')";
	executeupdate($sql);
  }
  
 
  $_SESSION['sess_msg'] = "$email has now been removed from this OPM email list.";
}

meta_redirect("unsubscribe_opm.php?$code");
?>
...
<div class="main">
This property can be accessed by any of the groups associated with its properties, OR by the following group:<br>
<form action="frame_prospect_groups_action.php" method="post" name="form1">
...
while($record = go_fetch_array($result)){
  $group_id = $record['id'];
  ?>
  <input type="checkbox" name="group_list[]" value="<?=$group_id?>"<?php if($groups==$group_id) echo " checked";?> onchange="group_check()"> 
  ...
  while($record_sub = go_fetch_array($result_sub)){
	...
	?>
	<input type="checkbox" name="subgroup_list[]" value="<?=$subgroup_id?>"<?php if($subgroups==$subgroup_id) echo " checked";?> onchange="subgroup_check()"> <?=stripslashes($record_sub['group_name'])?><br>
	<?php

...
$worksheet->write(0, 0,  "Invoice Date");
$worksheet->write(0, 1,  "Invoice Month");
$worksheet->write(0, 2,  "Dispatch ID");
$worksheet->write(0, 3,  "Invoice ID");
$worksheet->write(0, 4,  "Company ID");
$worksheet->write(0, 5,  "Company");
$worksheet->write(0, 6,  "Site");
$worksheet->write(0, 7,  "City");
$worksheet->write(0, 8,  "State");
$worksheet->write(0, 9,  "Section");
$worksheet->write(0, 10,  "Due Date");
$worksheet->write(0, 11,  "Status");
$worksheet->write(0, 12,  "Invoice");
if($custom_sd_field != "") $worksheet->write(0, 13, $custom_sd_field);
if($custom_sd_field2 != "") $worksheet->write(0, 14, $custom_sd_field2);

for($x=0;$x<sizeof($row);$x++){
  $counter = $x+1;
  $worksheet->write($counter, 0,  stripslashes($row[$x]['invoice_sent_date']));
  $worksheet->write($counter, 1,  "");
  $worksheet->write($counter, 2,  stripslashes($row[$x]['leak_id']));
  $worksheet->write($counter, 3,  stripslashes($row[$x]['invoice_id']));
  $worksheet->write($counter, 4,  stripslashes($row[$x]['cust_id']));
  $worksheet->write($counter, 5,  stripslashes($row[$x]['company_name']));
  $worksheet->write($counter, 6,  stripslashes($row[$x]['site_name']));
  $worksheet->write($counter, 7,  stripslashes($row[$x]['city']));
  $worksheet->write($counter, 8,  stripslashes($row[$x]['state']));
  $worksheet->write($counter, 9,  stripslashes($row[$x]['section_name']));
  $worksheet->write($counter, 10,  stripslashes($row[$x]['invoice_due_date']));
  $worksheet->write($counter, 11,  stripslashes($row[$x]['status']));
  $worksheet->write($counter, 12,  stripslashes($row[$x]['invoice_total']));
  if($custom_sd_field != "") $worksheet->write($counter, 13,  stripslashes($row[$x]['custom_field']));
  if($custom_sd_field2 != "") $worksheet->write($counter, 14,  stripslashes($row[$x]['custom_field2']));
}

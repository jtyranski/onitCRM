...
where 1=1 and status='Quoted' and prospect_id='$prospect_id' and display=1";
$result = executequery($sql);
...
$sql = "SELECT count(*) as count, sum(amount) as total from opportunities 
where 1=1 and status='Sold' and prospect_id='$prospect_id' and display=1";
$result = executequery($sql);
$record = go_fetch_array($result);
...
$sql = "SELECT count(*) as count, sum(amount) as total from opportunities 
where 1=1 and status='Dead' and prospect_id='$prospect_id' and display=1";
$result = executequery($sql);
$record = go_fetch_array($result);
...
<a href="opportunity_edit.php?prospect_id=<?=$prospect_id?>&property_id=<?=$corp_prop?>&opp_id=new" target="_parent">Add Opportunity</a>
<br>
View: 
...
where 1=1 $whereclause and prospect_id='$prospect_id' and display=1 order by lastaction desc";
$result = executequery($sql);
$counter=0;
while($record = go_fetch_array($result)){
  $counter++;
  ...
  <td valign="top">
  <?php if($corporate == 0){ ?>
    <a href="view_property.php?property_id=<?=$record['property_id']?>" target="_parent">
  <?php } ?>
  <?=$site_name?>
  <?php if($corporate == 0){ ?>
    </a>
  ...
  <a href="opportunity_edit.php?prospect_id=<?=$prospect_id?>&opp_id=<?=$record['opp_id']?>" target="_parent">edit</a> - 
  <a href="opportunity_delete.php?opp_id=<?=$record['opp_id']?>&prospect_id=<?=$prospect_id?>" target="_parent">delete</a>
  <?php } ?>
  </td>
  </tr>
  <?php
